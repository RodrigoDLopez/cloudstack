...
public class IpAddressManagerImpl extends ManagerBase implements IpAddressManager, Configurable {
        ...
        Transaction.execute(new TransactionCallbackWithExceptionNoReturn<InsufficientAddressCapacityException>() {
            ...
            public void doInTransactionWithoutResult(TransactionStatus status) throws InsufficientAddressCapacityException {
                ...
                if (network.getGateway() != null) {
                    configureIPV4Address(nic, requestedIpv4, vm, network, dc);
                }
                if(nic.getMacAddress()==null) {
                    s_logger.debug(String.format("Seems like you are deploying an instance with pure IPV6 Network. We need to get next available mac address for network ID [%d]", network.getId()));
                    configureIPV6Address(nic, network, requestedIpv6);
                }
                s_logger.debug(String.format("Setting ipv6 address for nic ID [%d] and network ID [%d]", nic.getId(), network.getId()));
                _ipv6Mgr.setNicIp6Address(nic, dc, network);
            }
        });
    }

    private void configureIPV6Address(NicProfile nic, Network network, String requestedIpv6) throws InsufficientAddressCapacityException {
        nic.setIPv6Gateway(network.getIp6Gateway());
        nic.setIPv6Cidr(network.getIp6Cidr());

        VlanVO vlan = _vlanDao.findByNetworkIdAndIpv6(network.getId(), requestedIpv6);
    ...
    private void configureIPV4Address(NicProfile nic, String requestedIpv4, VirtualMachineProfile vm, Network network, DataCenter dc) throws InsufficientAddressCapacityException {
